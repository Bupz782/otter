...
> Previously known as "Otter". A trustless DeFi automation protocol using ZKP delegation, MEV rebates, and proof-of-solvency.
...
- [EN ATTENTE] **US-358** : En tant que user, je veux voir les validations de l'intent :
  ...
  - ✓ Asset existant
  - ✓ Amount valide
  - Attention — Yield actuel : 2.8% (condition : >3%, pas encore remplie)
...
- [FAIT] **US-407** : En tant que backend, je veux un serveur HTTP Axum
- [FAIT] **US-408** : En tant qu'API, je veux exposer `POST /api/v1/intents/parse` (body: text → response: Intent)
- [FAIT] **US-409** : En tant qu'API, je veux exposer `POST /api/v1/intents` (create new intent)
- [FAIT] **US-410** : En tant qu'API, je veux exposer `GET /api/v1/intents` (list active intents)
- [FAIT] **US-411** : En tant qu'API, je veux exposer `GET /api/v1/intents/:id` (get intent details)
- [FAIT] **US-412** : En tant qu'API, je veux exposer `DELETE /api/v1/intents/:id` (cancel intent)
- [FAIT] **US-413** : En tant qu'API, je veux exposer `POST /api/v1/delegations` (create delegation)
- [FAIT] **US-414** : En tant qu'API, je veux exposer `GET /api/v1/delegations` (list delegations)
...
- [FAIT] **US-418** : En tant qu'API, je veux exposer `GET /api/v1/metrics` (stats & analytics)

### Epic 7.2 : Authentication & Security

- [FAIT] **US-419** : En tant qu'API, je veux authentifier via signature de message (EIP-4361 Sign-In with Ethereum)
- [FAIT] **US-420** : En tant qu'API, je veux générer un JWT après authentification
- [FAIT] **US-421** : En tant qu'API, je veux vérifier le JWT sur chaque requête protégée
- [FAIT] **US-422** : En tant qu'API, je veux rate limiting (100 req/min par user)
- [FAIT] **US-423** : En tant qu'API, je veux CORS configuré correctement (whitelist domains)
...
- [FAIT] **US-428** : En tant que backend, je veux un WebSocket server (Axum WS)
- [FAIT] **US-429** : En tant que WS server, je veux accepter les connexions des clients
- [EN ATTENTE] **US-430** : En tant que WS server, je veux authentifier les connexions (JWT dans handshake)
- [FAIT] **US-431** : En tant que WS server, je veux broadcaster les events aux clients connectés
- [EN COURS] **US-432** : En tant que WS server, je veux gérer les disconnections et reconnections

### Epic 7.5 : Monitoring & Observability

- [FAIT] **US-433** : En tant que système, je veux exposer des métriques Prometheus (`/metrics` endpoint)
- [EN COURS] **US-434** : En tant que système, je veux tracker les métriques :
...
- [FAIT] **US-437** : En tant que système, je veux logger en JSON (structured logging avec tracing-subscriber)
- [EN COURS] **US-438** : En tant que système, je veux inclure des contextes dans les logs (request_id, user_id, intent_id)
- [FAIT] **US-439** : En tant que système, je veux différents niveaux (ERROR, WARN, INFO, DEBUG, TRACE)
...
- [FAIT] **US-441** : En tant que système, je veux charger la config depuis un fichier TOML
- [FAIT] **US-442** : En tant que système, je veux override la config avec des env vars (12-factor app)
- [FAIT] **US-443** : En tant que système, je veux valider la config au démarrage (fail fast si invalid)
- [EN COURS] **US-444** : En tant que système, je veux supporter différents environnements (dev, staging, prod)

### Epic 7.8 : Database (persistence)

- [FAIT] **US-445** : En tant que système, je veux persister les intents dans une DB (PostgreSQL/SQLite)
- [FAIT] **US-446** : En tant que système, je veux persister les delegations
- [FAIT] **US-447** : En tant que système, je veux persister l'historique des transactions
- [EN ATTENTE] **US-448** : En tant que système, je veux persister les events de l'orchestrator (audit trail)
- [FAIT] **US-449** : En tant que système, je veux un `StoragePort` trait (`save_intent`, `get_intent`, `list_intents`, etc.)
- [FAIT] **US-450** : En tant qu'infra, je veux un `PostgresAdapter` et `SqliteAdapter` implémentant `StoragePort`
- [FAIT] **US-451** : En tant que système, je veux des migrations DB (sqlx ou diesel migrations)
...
- [FAIT] **US-457** : En tant que système, je veux un endpoint `GET /health` (retourne status: UP/DOWN)
- [FAIT] **US-458** : En tant que système, je veux checker les dépendances (DB, RPC node, Oracle, etc.)
- [FAIT] **US-459** : En tant que système, je veux un endpoint `GET /ready` (readiness probe pour Kubernetes)
...
- Passe `[EN ATTENTE]` en `[EN COURS]` quand tu commences la story
- Passe `[EN COURS]` en `[FAIT]` quand elle est terminée et testée
