...
impl OtterBridgeAdapter {
    pub fn new(rpc_url: &str, bridge_address: &str, private_key_hex: &str, chain_id: u64) -> Result<Self, BridgeError> {
...
impl BridgePort for OtterBridgeAdapter {
    async fn lock(&self, _user: String, amount: String, destination_chain_id: u64) -> Result<BridgeLockResult, BridgeError> {
        let amount = amount.parse::<U256>().map_err(|e| BridgeError::InvalidInput(format!("invalid amount: {}", e)))?;

        let signer = PrivateKeySigner::from_str(&self.private_key_hex)
            .map_err(|e| BridgeError::InvalidInput(format!("invalid private key: {}", e)))?;
        let url = self.rpc_url.parse::<reqwest::Url>().map_err(|e| BridgeError::InvalidInput(e.to_string()))?;
        let provider = ProviderBuilder::new().with_chain_id(self.chain_id).wallet(EthereumWallet::from(signer)).on_http(url);

        let contract = IOtterBridge::new(self.bridge_address, provider);
        let call = contract.lock(amount, U256::from(destination_chain_id));
        let tx = call.send().await.map_err(|e| BridgeError::Contract(e.to_string()))?;
        let receipt = tx.get_receipt().await.map_err(|e| BridgeError::Contract(e.to_string()))?;
    ...
    async fn mint(&self, user: String, amount: String, bridge_id: String) -> Result<String, BridgeError> {
        let user = Address::from_str(&user).map_err(|e| BridgeError::InvalidInput(format!("invalid user address: {}", e)))?;
        let amount = amount.parse::<U256>().map_err(|e| BridgeError::InvalidInput(format!("invalid amount: {}", e)))?;
        let bridge_id = B256::from_str(&bridge_id).map_err(|e| BridgeError::InvalidInput(format!("invalid bridge id: {}", e)))?;
