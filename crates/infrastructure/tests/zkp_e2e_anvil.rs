...
#[test]
fn verify_delegation_proof_on_anvil() {
    let rpc_url = match std::env::var("OTTER_TEST_RPC_URL") {
        Ok(url) => url,
        Err(_) => {
            eprintln!("Skipping zkp_e2e_anvil: OTTER_TEST_RPC_URL not set");
            return;
        }
    };
    let vault_address = match std::env::var("OTTER_TEST_VAULT_ADDRESS") {
        Ok(addr) => addr,
        Err(_) => {
            eprintln!("Skipping zkp_e2e_anvil: OTTER_TEST_VAULT_ADDRESS not set");
            return;
        }
    };
    let private_key = match std::env::var("OTTER_TEST_PRIVATE_KEY") {
        Ok(key) => key,
        Err(_) => {
            eprintln!("Skipping zkp_e2e_anvil: OTTER_TEST_PRIVATE_KEY not set");
            return;
        }
    };
    ...
    if bb_bin.is_none() {
        eprintln!("Skipping zkp_e2e_anvil: BB_BIN not found");
        return;
    }
    ...
    assert_eq!(
        ...
        "public inputs must match the on-chain format"
    );

    let evm = AlloyEvmAdapter::new(rpc_url, &private_key, &vault_address)
        .expect("evm adapter should build");
    let valid = evm
        .verify_onchain(&proof)
        .expect("on-chain verification should not error");
    assert!(valid, "proof should be valid on-chain");
}

#[test]
fn local_public_inputs_match_on_chain_format() {
    // This test always runs and ensures the serialization is correct even
