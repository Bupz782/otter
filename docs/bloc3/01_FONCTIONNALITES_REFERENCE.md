...
| F10 | Agent d'exécution opéré par Otter (page transparence : identité, clé publique, preuves) — l'utilisateur délègue, il ne crée jamais d'agent — + templates d'intents forkables | commit `cfbf746` + refonte agent (09/2026) ; pages `/app/agents`, `/app/strategies` | PAGE (données marquées démo dans l'API — header X-Demo-Data, à assumer) |
