...
| F10 | Agent d'exécution connecté au déploiement (page transparence : identité, clé publique, preuves) — Otter est une infra agent-agnostic, elle ne crée pas d'agents — + templates d'intents forkables | commit `cfbf746` + refonte agent-agnostic (09/2026) ; pages `/app/agents`, `/app/strategies` | PAGE (données marquées démo dans l'API — header X-Demo-Data, à assumer) |
