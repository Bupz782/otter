# Cahier de recettes — Projet Otter (ex-Otter)
...
- **Backend Rust/Axum** en architecture hexagonale (`crates/domain`, `crates/application`, `crates/infrastructure`, `crates/interfaces`) — API REST `otter_api`, CLI `otter_cli` ;
...
Le périmètre couvre les fonctionnalités **réellement implémentées** à la date de la recette, alignées sur les user stories marquées dans `BACKLOG.md` (vagues 0, 1 et 7 notamment). Les vagues marquées [EN ATTENTE] / [FUTUR] dans `BACKLOG.md` (FHE, MEV rebates avancés, market place d'agents réelle, etc.) sont **hors périmètre** : aucun scénario n'est créé pour des fonctionnalités non implémentées.
...
- les données `agent`, `strategies` exposées par l'API sont des **jeux de données embarqués** (données de démonstration), pas des données persistées ;
...
| ID | Fonctionnalité / User Story | Préconditions | Étapes | Résultat attendu | Résultat obtenu | Statut |
|---|---|---|---|---|---|---|
| REC-F-022 | Génération de la preuve Noir (witness + UltraHonk) | `nargo` + `bb` installés, délégation signée | `otter_cli prove "lend 100 USDC on Aave" --private-key 0x…` (ou `NoirAdapter` via l'agent) | `proof.bin` + `public_inputs.bin` générés ; la witness atteste : hash délégation, signature ECDSA valide, intent autorisé, montant ≤ max, protocole whitelisté, non expiré, nonce conforme | Preuve générée pour une délégation valide | Conforme (automatisé : `crates/infrastructure/tests/zkp_noir.rs` — `noir_adapter_generates_witness_for_valid_delegation`) |
...
| REC-F-030 | Diffusion temps réel via WebSocket (US-428/429/431) | Client connecté à `GET /api/v1/ws` | Publier des événements (price update, condition, exécution) | Chaque événement sérialisé JSON est broadcasté aux clients connectés | Événements reçus en temps réel côté frontend | Conforme (manuel ; code : `ws_handler`/`handle_socket` dans `crates/interfaces/src/bin/otter_api.rs`) |

### 3.8 Endpoints complémentaires de l'API

| ID | Fonctionnalité / User Story | Préconditions | Étapes | Résultat attendu | Résultat obtenu | Statut |
|---|---|---|---|---|---|---|
| REC-F-031 | Endpoints agent / stratégies | — | `GET /api/v1/agents`, `/api/v1/agents/:id`, `/api/v1/agents/:id/pubkey`, `/api/v1/strategies` | HTTP 200, données de démonstration cohérentes (1 agent « Otter Agent » opéré par le protocole, 3 stratégies templates) ; 404 sur agent inconnu ; pubkey agent exposée si clé configurée | Réponses conformes (données embarquées de démo — voir anomalie A2) | Conforme (automatisé : `crates/interfaces/src/bin/otter_api.rs` — handlers testés via router) |
| REC-F-032 | Endpoint portfolio | — | `GET /api/v1/portfolio` | HTTP 200 : adresse du signataire, solde (réel si exécution activée, zéros sinon), positions | Réponse conforme au mode de fonctionnement | Conforme (automatisé : `crates/interfaces/src/bin/otter_api.rs`) |
| REC-F-033 | Endpoints proofs / executions | ≥ 1 exécution persistée | `GET /api/v1/proofs`, `GET /api/v1/executions` | HTTP 200, historique des exécutions (tx_hash, statut, gas) alimenté depuis la base | Historique conforme aux enregistrements | Conforme (automatisé : `crates/interfaces/src/bin/otter_api.rs` ; `crates/infrastructure/tests/storage_sqlite.rs` — `save_and_list_executions`) |

### 3.9 Parcours CLI (`crates/interfaces/src/bin/otter_cli.rs`)

| ID | Fonctionnalité / User Story | Préconditions | Étapes | Résultat attendu | Résultat obtenu | Statut |
|---|---|---|---|---|---|---|
| REC-F-034 | `otter_cli parse` / `plan` (US-047/048) | Binaire compilé (`cargo build --bin otter_cli`) | `otter_cli parse "lend 1000 USDC on Aave"` ; `otter_cli plan "swap 1 ETH for USDC on Uniswap"` | Intent structuré, puis plan multi-steps affichés en sortie | Sorties conformes au domain model | Conforme (manuel ; logique couverte par `crates/infrastructure/src/parsers/regex_parser.rs` et `crates/application/src/services/strategy_planner.rs`) |
| REC-F-035 | `otter_cli execute` en mode mock | — | `otter_cli execute "swap 1000 USDC for ETH on Uniswap"` (sans `--vault`) | Pipeline complet simulé (mock ZKP + mock EVM) avec affichage des transitions | Simulation conforme, sans dépendance réseau | Conforme (manuel ; `MockZkpAdapter`, `MockEvmAdapter`) |
| REC-F-036 | `otter_cli execute` / `prove` / `verify-onchain` on-chain | Anvil + vault déployé, clé fondée | `otter_cli execute "…" --rpc-url … --private-key … --vault 0x… --delegate`, puis `verify-onchain --proof proof.bin …` | Délégation enregistrée, preuve réelle soumise et vérifiée on-chain | Flux E2E CLI validé (script `lab/zkp_e2e.sh`) | Conforme (manuel + automatisé : `lab/zkp_e2e.sh`, `crates/infrastructure/tests/e2e_anvil_flow.rs`) |
| REC-F-037 | `otter_cli start` / `status` | RPC joignable | `otter_cli start "lend 100 USDC on Aave if yield > 1" --network sepolia`, puis `otter_cli status` | Daemon de surveillance démarré (intervalle configurable), état et intents actifs affichés | Daemon fonctionnel, état consultable | Conforme (manuel) |
...
| REC-F-042 | Création de délégation (`CreateDelegationPage`) | Wallet connecté | Vérifier l'agent unique présélectionné, montants max par intent, protocoles autorisés, chaînes, expiration (jours) ; signer avec le wallet | Formulaire validé, délégation signée puis soumise à `POST /api/v1/delegation`, hash affiché | Délégation créée depuis l'UI | Conforme (manuel ; `frontend/src/lib/delegation.ts`) |
| REC-F-043 | Pages Delegations / Agent / Proofs / Settings | — | Parcourir les pages correspondantes | Données affichées depuis l'API (délégations, agent, preuves), états vides/erreur gérés (`EmptyState`, `ErrorState`) | Pages rendues et cohérentes avec les endpoints | Conforme (manuel) |

### 3.11 Endpoints d'observabilité et déploiement

| ID | Fonctionnalité / User Story | Préconditions | Étapes | Résultat attendu | Résultat obtenu | Statut |
|---|---|---|---|---|---|---|
| REC-F-044 | `GET /health` et `GET /health/live` (US-457) | API démarrée | `curl localhost:3001/health` | HTTP 200 `{"status":"up","version":…,"timestamp":…}` ; `/health/live` → `ok` | Endpoints de santé conformes | Conforme (automatisé : `crates/interfaces/src/bin/otter_api.rs` — `health_endpoint_returns_status_and_version` ; `scripts/smoke-test.sh`) |
| REC-F-045 | `GET /ready` (US-458/459) | API démarrée, DB/RPC/oracle joignables ou non | `curl localhost:3001/ready` | HTTP 200 `{"status":"ready"}` si storage + RPC + oracle OK ; HTTP 503 avec détail des échecs sinon | Readiness conforme (200 nominal, 503 dégradé) | Conforme (automatisé : `scripts/smoke-test.sh` — polling 200/503 ; code : `ready` dans `otter_api.rs`) |
| REC-F-046 | `GET /metrics` Prometheus (US-418/433) | `OTTER_METRICS_ENABLED=true` | `curl localhost:3001/metrics` | Exposition au format texte Prometheus : `otter_price_updates_total`, `otter_executions_total`, `otter_gas_used_total`, `otter_active_intents`, `otter_proof_*_seconds`, etc. ; HTTP 404 si métriques désactivées | Métriques exposées au format attendu | Conforme (automatisé : `crates/interfaces/src/bin/otter_api.rs` — `metrics_endpoint_returns_prometheus_format`, `metrics_endpoint_disabled_returns_not_found`) |
...
- **Rust** : ≈ 272 fonctions de test sur le workspace (`cargo test --workspace`) ;
...
|---|---|---|---|
| A1 | Moyenne | Le endpoint `POST /api/v1/intents/parse` n'appliquait pas `validate_intent_text` : un texte > 2000 caractères y était accepté, contrairement à `POST /api/v1/intents` (REC-SEC-009). **Résolu** : `validate_intent_text` est appelée dans `parse_intent` ; tests `parse_intent_rejects_long_text` et `parse_intent_rejects_empty_text` (HTTP 400, même code que sur `/intents`). | `crates/interfaces/src/bin/otter_api.rs` — `parse_intent` |
| A2 | Faible | Les endpoints `/agents`, `/strategies` retournent des données de démonstration embarquées (`default_agents()`, `default_strategies()`) et `/proofs` ajoute une preuve de solvency factice codée en dur. À remplacer par des données persistées avant mise en production. **Étiquetage explicite ajouté** (header `X-Demo-Data: true` et champ `demo: true` sur les réponses, badge « Demo data » dans l'UI) ; remplacement par données on-chain/persistées tracé — retrait obligatoire avant mainnet. *Mise à jour 09/2026 : l'endpoint `/leaderboard` a été retiré avec la refonte vers l'agent unique.* | `crates/interfaces/src/bin/otter_api.rs` |
| A3 | Faible | Parsing LLM : le module wrappe un modèle local llama.cpp/GGUF et non l'API Claude visée par US-028/029/030 (choix assumé : confidentialité, hors-ligne) ; le `HybridParser` n'était pas branché sur l'API. **Résolu** : `build_orchestrator` instancie le `HybridParser` (fallback LLM → regex) lorsqu'un modèle GGUF est présent et chargeable, sinon le `RegexParser` seul ; le démarrage sans modèle reste le cas par défaut (test `build_intent_parser_falls_back_to_regex_without_model`). | `crates/infrastructure/src/llm/`, `crates/interfaces/src/bin/otter_api.rs` — `build_orchestrator`, `build_intent_parser` |
| A4 | Faible | Rate limiting implémenté **par adresse IP** alors que US-422 spécifie « 100 req/min **par user** ». **Résolu** : la clé de comptage est le `sub` du JWT quand un token valide est présent, l'IP sinon ; test `rate_limit_scoped_per_user` (quotas différenciés par utilisateur depuis la même IP, bucket IP conservé pour l'anonyme). | `crates/interfaces/src/bin/otter_api.rs` — `rate_limit_middleware`, `rate_limit_key` |
...
| A7 | Faible | Vérification SIWE de bout en bout (signature réelle d'un wallet) non couverte par un test automatisé ; couverte manuellement (REC-F-002). **Résolu** : test d'intégration `siwe_end_to_end_real_signature` (challenge → signature EIP-191 réelle via `k256` → `/auth/verify` → JWT dont le `sub` est l'adresse du signataire → appel protégé accepté) et test unitaire `verify_signature_accepts_real_eip191_signature` dans `auth.rs`. Au passage, la vérification de signature a été rendue synchrone (`verify_eip191`) car le `block_on` imbriqué faisait échouer l'appel dans le runtime async, et le `sub` du JWT porte désormais l'adresse EIP-55 (et non le debug du tableau d'octets). | `crates/interfaces/src/auth.rs`, `crates/interfaces/src/bin/otter_api.rs` |

### 6.4 Conclusion

L'ensemble des fonctionnalités implémentées et revendiquées dans `BACKLOG.md` (stories [FAIT]) est couvert par au moins un scénario de ce cahier. La chaîne de valeur critique — authentification SIWE/JWT, parsing d'intention, planification, délégation signée, preuve ZK Noir, vérification et exécution on-chain avec limites et anti-rejeu — est vérifiée de bout en bout, majoritairement par des tests automatisés traçables. Les écarts identifiés (A1–A7) sont de gravité faible à moyenne et n'entament pas la validité des garanties de sécurité du flux de délégation ; A1, A3, A4, A6 et A7 sont résolus, A2 est accepté pour le MVP (données de démonstration à persister avant production) et A5 reste à traiter avant mainnet — voir `docs/PLAN_CORRECTION_BOGUES.md` (§5).
