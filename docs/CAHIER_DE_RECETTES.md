...
| A2 | Faible | Les endpoints `/agents`, `/strategies`, `/leaderboard` retournent des données de démonstration embarquées (`default_agents()`, `default_strategies()`) et `/proofs` ajoute une preuve de solvency factice codée en dur. À remplacer par des données persistées avant mise en production. | `crates/interfaces/src/bin/otter_api.rs` |
